...
<form action="updateConfirm" method="post">
  ...
  <fieldset class="col">
    ...
    <div>
      <label>PASS</label><input type="text" value="<%=session.getAttribute("Pass")%>" disabled>
    </div>
  </fieldset>
  ...
  <fieldset class="col label-110">
    <legend>変更後</legend>
    <div>
      <label>名前</label><input type="text" name="newName" value="<%=session.getAttribute("Name") %>" readonly>
    </div>
    <div>
      <label>TEL</label><input type="text" name="newTel" value="<%=session.getAttribute("Tel") %>" readonly>
    </div>
    <div>
      <label>PASS(再入力)</label><input type="text" name="repass" value="<%=session.getAttribute("oldPass")%>">
    </div>
  </fieldset>
